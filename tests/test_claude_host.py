...
from the_big_learn.claude_host import default_claude_target, install_claude_skills, source_skill_dirs


class ClaudeHostTests(unittest.TestCase):
    def test_default_claude_target_looks_like_claude_skill_home(self) -> None:
        self.assertTrue(str(default_claude_target()).endswith(".claude/skills"))
    ...
    def test_install_claude_skills_copies_skills(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp)
            installed = install_claude_skills(target=target)
            ...
            self.assertIn("bundled full-book source catalogs plus raw chapter files for the current curriculum set", launcher_content)
            ...
            self.assertIn("before invoking web discovery for a chosen book", launcher_content)
            ...
            self.assertIn("supported through bundled local source catalogs plus raw chapters", launcher_content)
            self.assertIn("If a saved or bundled source catalog already exists", launcher_content)
            self.assertIn("bundled, saved, or live-source catalog chapter numbers", launcher_content)
            self.assertIn("curriculum books that are packaged locally, prefer bundled `books/`", launcher_content)
            ...
            self.assertIn("bundled curriculum `books/` data where it is packaged", launcher_content)
