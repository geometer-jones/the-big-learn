...
class CodexHostTests(unittest.TestCase):
    ...
    def test_install_codex_skills_copies_skills(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("10. Chengyu Catalog", launcher_content)
            self.assertIn("references/hanzi/modern-common-components-gf0014-2009-grouped.json", launcher_content)
            self.assertIn("If the learner chooses `0`, do not open a chapter menu", launcher_content)
            self.assertIn("Component Reference Turn", launcher_content)
            self.assertIn("compact table with columns `Chinese Forms`, `Reading`, `English Meaning`, and `Example Words`", launcher_content)
            self.assertIn("render `Reading` as `pinyin (zhuyin)`", launcher_content)
            self.assertIn("default browse view is frequency-ranked in pages of 100 rows", launcher_content)
            self.assertIn("start with the first 100 grouped components", launcher_content)
            self.assertIn("descending grouped `occurrence_count`", launcher_content)
            self.assertIn("let `+` or `next` mean the next frequency-ranked page", launcher_content)
            self.assertIn("live assistant help rather than pre-encoded repository data", launcher_content)
            ...
            self.assertIn("point `THE_BIG_LEARN_STATE_DIR` elsewhere if they want a different storage location", launcher_content)
            self.assertIn("DESIGN.md", launcher_content)
            self.assertIn("books", launcher_content)
            ...
            self.assertIn("Do not substitute any legacy repo-local starter annotation slice", launcher_content)
            ...
            self.assertIn("the learner can rewrite their personal translation if they wish", launcher_content)
            ...
            self.assertIn("specific curriculum book, chapter, or line", launcher_content)
            self.assertIn("go straight to that book's chapter menu without first showing the opening book menu", launcher_content)
            ...
            self.assertIn("begin the reading pass without first showing the opening book menu or the chapter menu", launcher_content)
            self.assertIn("begin the reading pass at that line without first showing the opening book menu or the chapter menu", launcher_content)
            self.assertIn("keep the first rendered focus on the requested line instead of forcing a menu detour", launcher_content)
            ...
            self.assertIn("Do not swap the shipped `Da Xue` chapter flow into a repo-local starter annotation slice", launcher_content)
