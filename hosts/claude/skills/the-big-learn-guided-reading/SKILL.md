...
description: Guide a learner through The Big Learn reading workflow inside Claude Code. Use when the user wants to read Da Xue or another curriculum text through bundled raw chapters, live assistant support, and saved generated help on reread, while handling learner questions directly inside the current line discussion.
disable-model-invocation: true
---

# The Big Learn Guided Reading

Use this skill when the user wants a Claude Code Chinese learning session.
...
- under each chapter title in the menu, render the title as a miniature reference line using the same order as regular line rendering: full-title Chinese, full-title English, then a char-by-char table in this order: Index, Chinese, Reading, English Definition, Chinese Phrase, English Phrase Translation; do not open the learner translation and response loop for titles
...
- when a raw-source reading unit already carries saved generated layers, segments, or character glosses from an earlier session, you may render that stored help in the line shell from `DESIGN.md`: `Your translation?`, full-line simplified Hanzi, full-line English, then a six-column table in this order: Index, Chinese, Reading, English Definition, Chinese Phrase, English Phrase Translation, using rowspans for encoded multi-character phrases, numbering each character row from 1 through the full line length so the learner can reference any visible position directly, keeping simplified primary throughout the char-by-char table while appending traditional in parens only where it differs, keeping pinyin primary with zhuyin in parens, and using semicolon-separated context-free character definitions in the `English Definition` column, then the same full line again in English and simplified Hanzi, then line location and identity, then `Your translation?`; if a learner translation is already recorded for that line, show it immediately under that bottom prompt
- if the host surface is narrow or the six-column table becomes hard to read, switch to a stacked per-character list that preserves the same information order instead of forcing a cramped table
