...
from .data_paths import BOOKS_DIRNAME, candidate_books_dirs, data_root, preferred_books_dir
from .flashcards import reconstruct_line_from_character_index
from .updates import state_dir


SOURCE_STORE_DIRNAME = BOOKS_DIRNAME
...
def _candidate_cache_dirs(books_roots: tuple[Path, ...], source_url: str) -> list[Path]:
    candidates: list[Path] = []
    for books_root in books_roots:
        for dirname in (_source_store_dirname(source_url), _source_store_key(source_url)):
            candidate = books_root / dirname
...
def _state_books_roots() -> tuple[Path, ...]:
    return candidate_books_dirs(state_dir())


def _packaged_books_roots() -> tuple[Path, ...]:
    return candidate_books_dirs(data_root())


def _candidate_state_cache_dirs(source_url: str) -> list[Path]:
    return _candidate_cache_dirs(_state_books_roots(), source_url)


def _candidate_packaged_cache_dirs(source_url: str) -> list[Path]:
    return _candidate_cache_dirs(_packaged_books_roots(), source_url)
...
def packaged_source_cache_dir(source_url: str) -> Path:
    return preferred_books_dir(data_root()) / _source_store_dirname(source_url)
...
def _read_cached_html(source_url: str) -> str | None:
    for path in _candidate_cached_paths(
        *(cache_dir / HTML_CACHE_FILE for cache_dir in _candidate_state_cache_dirs(source_url)),
        *(cache_dir / HTML_CACHE_FILE for cache_dir in _candidate_packaged_cache_dirs(source_url)),
    ):
        try:
...
def load_saved_source_catalog(source_url: str) -> dict[str, Any] | None:
    catalog_file, payload = _read_first_json_file(
        *(cache_dir / CATALOG_CACHE_FILE for cache_dir in _candidate_state_cache_dirs(source_url)),
        *(cache_dir / CATALOG_CACHE_FILE for cache_dir in _candidate_packaged_cache_dirs(source_url)),
    )
...
def load_saved_source_chapter(source_url: str, chapter_id: str) -> dict[str, Any] | None:
    chapter_file, payload = _read_first_json_file(
        *(cache_dir / CHAPTERS_DIRNAME / f"{chapter_id}.json" for cache_dir in _candidate_state_cache_dirs(source_url)),
        *(
            cache_dir / CHAPTERS_DIRNAME / f"{chapter_id}.json"
            for cache_dir in _candidate_packaged_cache_dirs(source_url)
        ),
    )
